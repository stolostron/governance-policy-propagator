...
// objet with the handleDecision method. This is used instead of making this a method on the
	...
	for _, pb := range pbList.Items {
		...
		for _, subject := range subjects {
			...
			for result := range resultsChan {
				...
				// the channels and stop blocking in the this goroutine.
				if len(allDecisions) == len(decisions) {
					close(decisionsChan)
					close(resultsChan)
